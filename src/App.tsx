...
function ThemeToggle() {
  ...
  return (
    <button
      ...
      title={t("nav.theme", { mode: t(`theme.${pref}`) })}
    >
      {THEME_ICONS[pref]}
    </button>
  )
...
function LanguageToggle() {
  ...
  return (
    <button
      onClick={cycle}
      className="px-1.5 h-6 rounded-md border border-border font-mono text-[10px] text-muted-foreground hover:text-foreground hover:border-primary/50 transition-colors"
      title={t("lang.title", { lang: lang === "en" ? "English" : "中文" })}
    >
      {lang === "en" ? "EN" : "中"}
    </button>
  )

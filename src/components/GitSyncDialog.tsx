...
export function GitSyncDialog({ open, skills, onClose, onChanged }: GitSyncDialogProps) {
  ...
  useEffect(() => {
    ...
    invoke<GitRemotesConfig>("get_git_config")
      .then(setConfig)
      .catch((e) => toast(`Failed to load git config: ${e}`, "error"))
  }, [open])
  ...
  const removeRemote = async (name: string) => {
    ...
    try {
      ...
      toast(`Failed to remove: ${e}`, "error")
    }
  }
  ...
  const setSkillScope = async (skill: string, scope: string | null) => {
    try {
      ...
      toast(`Failed to set scope: ${e}`, "error")
    }
  }
  ...
  const publish = async (scope: string) => {
    ...
    try {
      ...
      const text = r.committed
        ? `Committed and pushed ${r.skills.length} skill${r.skills.length === 1 ? "" : "s"}`
        : `No changes to push (${r.skills.length} skill${r.skills.length === 1 ? "" : "s"} in scope)`
  ...
  const pull = async (scope: string) => {
    ...
    try {
      ...
      const text =
        r.updated.length > 0
          ? `Updated ${r.updated.length} skill${r.updated.length === 1 ? "" : "s"} from remote`
          : "Remote has no skills yet"
      ...
      <DialogContent className="sm:max-w-[600px] flex flex-col">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <GitBranch size={14} /> Git Backup &amp; Share
          </DialogTitle>
          <DialogDescription>
            Publish skills to separate git remotes by scope — keep personal and company skills in
            their own repositories. Uses your local git credentials.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="max-h-[440px] overflow-y-auto -mx-1 px-1 space-y-3">
          {config.remotes.length === 0 && (
            <p className="text-xs text-muted-foreground bg-muted/40 border border-border rounded-md p-3">
              No scopes yet. Add a git repository below (e.g. your personal skills repo), then
              assign skills to scopes and publish. Skills are mirrored, committed and pushed — the
              remote receives only its own scope's skills.
            </p>
          )}
              ...
              <div key={r.name} className="border border-border rounded-lg p-3 space-y-2">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <span className="text-[10px] text-muted-foreground shrink-0">
                    {inScope.length} skill{inScope.length === 1 ? "" : "s"}
                  </span>
                  <Button
                    variant="ghost"
                    size="icon"
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    <CloudUpload size={12} /> {busy ? "..." : "Publish"}
                  </Button>
                  <Button
                    variant="ghost"
                    size="sm"
          ...
          {config.remotes.length < 8 && (
            <div className="border border-dashed border-border rounded-lg p-3 space-y-2">
              <Label className="text-[11px] text-muted-foreground">Add scope</Label>
              <div className="flex gap-2">
                <Input
                  value={form.name}
                  onChange={(e) => setForm({ ...form, name: e.target.value })}
                  placeholder="scope (work)"
          ...
          {config.remotes.length > 0 && skills.length > 0 && (
            <div className="border border-border rounded-lg p-3 space-y-1.5">
              <p className="text-[11px] text-muted-foreground">
                Skill scopes — unassigned skills follow the default scope
